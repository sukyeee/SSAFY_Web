...
<body>
  <div class="container-fluid">
    ...
    <div id="board-list" class="footer-card">
		...
		<button class="btn btn-sm btn-outline-secondary" type="button" id="eventSetting" ><a href="<%=contextPath %>/jsp/board/boardMain.jsp">이벤트 관리</a></button>
      <% } %>
  ...
  async function boardList() {
			let url = '<%=contextPath%>/board/boardListCustomer';
  ...
  function makeListHtml(list){
		...
		if(listHtml.length == 0)
			...
			makeListHtmlEventHandler(list);
		}
		
	}
  function makeListHtmlEventHandler(list){
		document.querySelectorAll("#board-list > table > tbody > tr").forEach( el => {
			el.onclick = function(){
				let no = this.getAttribute("data-eventKey");
				
				let url = '<%= contextPath%>/board/boardPage';
				let urlParams = '?eventKey=' + no;
				
				window.location.href=url+urlParams;
					//fetch( url + urlParams, fetchOptions );
				
				
				//window.location.href="<%= contextPath%>/boardPage";
			}
		})
	}
